...
//		path: "",		 		// (required) path to request
	...
	es.ui.ClusterOverview = es.ui.Page.extend({
		...
		_replica_handler: function(index) {
			var fields = new acx.ux.FieldCollection({
				fields: [
					new acx.ui.TextField({ label: acx.text("AliasForm.ChangeReplicaMessage"), name: "replicas", require: true })
				]
			});
			var dialog = new acx.ui.DialogPanel({
				title: acx.text("AliasForm.ReplicasForIndexName", index.name),
				body: new acx.ui.PanelForm({ fields: fields }),
				onCommit: function(panel, args) {
					if(fields.validate()) {
						var data = fields.getData();
					...
					new acx.ui.MenuButton({
						...
						menu: new acx.ui.MenuPanel({
							items: [
								...
								{ text: acx.text("IndexActionsMenu.ClearCache"), onclick: function() { this._postIndexAction_handler("_cache/clear", index, false); }.bind(this) },
								{ text: acx.text("IndexActionsMenu.Replicas"), onclick: function() { this._replica_handler(index); }.bind(this) }
							]
						})
					})
				] }
